...
- **`shell`:** Strictly limited to non-destructive read-only commands (`git status`, `git log`, `git rev-parse`, `git rev-list`, and `git stash list`). It cannot run `git commit`, `git checkout`, etc.
